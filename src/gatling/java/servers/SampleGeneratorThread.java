...
public class SampleGeneratorThread extends Thread {
    private static final Logging log = Logging.getLogging(SampleGeneratorThread.class);
    ...
    @Override
    public void run() {
        log.debug("SampleGeneratorThread is running");
        ...
        DataProvider[] providers = {
                new RandomRecordsProvider(scheme.getRecord(0), symbols, 10),
                new RandomRecordsProvider(scheme.getRecord(1), symbols, 3),
                new RandomRecordsProvider(scheme.getRecord(2), new String[]{symbols[0]}, 3)
        };
        RecordBuffer buffer = new RecordBuffer();
        try {
            while (!Thread.interrupted()) {
                log.debug("Providing records");
